    ...
    try {
      ...
      if (useDirectSupabase) {
        ...
        try {
          ...
          nextProfile = response?.data || profileForm;
        } catch (apiErr) {
          // If API fails but we have local data, still consider it a local save
          console.warn("API save failed, using local profile:", apiErr?.message);
          nextProfile = profileForm;
        }
      }

      // Update state with either server or local profile
      ...
      // Last resort: save locally anyway
      setProfile(profileForm);
      setProfileForm(profileForm);
      localStorage.setItem("userProfile", JSON.stringify(profileForm));
      setShowProfileModal(false);
      setError(""); // Don't show error since we saved locally
    }
  };
